...
![Add NuGet packages]({{ page.tutorialroot }}images/add-nuget-packages.png "Add NuGet packages")

![Updated references]({{ page.tutorialroot }}images/updated-references.png "Updated references")
...
-
Now that bootstrapping is taken care of, it's time to configure [auto-routing]({{ page.documentationroot }}/auto_routing.html). Auto-routing will automatically generate [routes]({{ page.documentationroot }}/routes.html) given a set of conventions.
...
```Route```'s constructor requires a name. In the below code, we choose to name routes after the endpoint class' namespace and class name. Internally, a new [name mapper]({{ page.documentationroot }}/name_mappers.html) is added.

{% highlight csharp %}
.NameAfterRelativeClassNamespaceAndClassName(endpointNamespace)
{% endhighlight %}

```Route```'s constructor also requires an ID. The only time route IDs are normally used is when resolving URLs, and even then it's preferable to resolve routes by name for clarity. In the below code, we choose to generate route IDs randomly with the provided ```IGuidFactory``` implementation. Internally, a new [ID mapper]({{ page.documentationroot }}/id_mappers.html) is added.

{% highlight csharp %}
.IdRandomly(guidFactory)
{% endhighlight %}

```Route```'s constructor also requires a resolved relative URL.  In the below code, we choose to assign resolved relative URLs based on the endpoint class' namespace and class name. Internally, a new [resolved relative URL mapper]({{ page.documentationroot }}resolved_relative_url_mappers.html) is added.
...
In most cases, routes will be returning instances of [```IResponse```]({{ page.documentationroot }}responses.html). In the below code, we choose to invoke the endpoint methods and return whatever ```IResponse``` instance they return. Internally, a new [response mapper]({{ page.documentationroot }}response_mappers.html) is added.
...
![Route test]({{ page.tutorialroot }}images/route-test.png "Route test")

Step 11 - Add Diagnostic Routes, If Desired
-
Adding [diagnostic]({{ page.documentationroot }}diagnostics.html) routes causes significant changes to occur in your configuration class, so replace its code with the following:
...
There are some important changes here. Notice how the [```IRouteCollection```]({{ page.documentationroot }}route_collections.html) instance is now a field instead of a variable. [```UrlResolver```]({{ page.documentationroot }}url_resolvers.html) requires a route collection so that it may search the collection for routes that match a particular name or ID. ```RoutingDiagnosticConfiguration``` renders a diagnostics page that displays all your configured routes. We need to pass the route collection to both of these objects. However, at the time we create instances of ```UrlResolver``` and ```RoutingDiagnosticConfiguration```, the route collection has not yet been generated. In the below code, we create an ```IRouteCollection``` field that allows us to provide the route collection as a delegate.
...
![Diagnostics route test]({{ page.tutorialroot }}images/diagnostics-route-test.png "Diagnostics route test")